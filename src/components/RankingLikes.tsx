...
export default function RankingLikes({ meta, compact = false }: { meta: RankingMeta; compact?: boolean }) {
  ...
  return <div className={compact ? "" : "mt-5"}>
    <div className={compact ? "flex items-center" : "flex flex-wrap items-center gap-3"}>
      <button type="button" onClick={toggle} disabled={loading || busy || owner || (!!user && !ready)} aria-pressed={liked} aria-label={owner ? "You cannot like your own ranking" : liked ? "Unlike ranking" : user ? "Like ranking" : "Sign in to like ranking"} title={owner ? "You cannot like your own ranking" : undefined} className={`inline-flex min-h-10 cursor-pointer items-center gap-2 border px-3 text-sm transition-colors disabled:cursor-default disabled:opacity-50 ${compact ? "rounded-l-xl border-r-0" : "rounded-xl"} ${liked ? "border-pink-500/30 bg-pink-500/10 text-pink-400" : compact ? "border-white/15 bg-white/[0.03] text-slate-200 hover:bg-white/10" : "border-[var(--border)] text-[var(--text-muted)] hover:bg-[var(--surface-hover)]"}`}>
        ...
        {!compact && (busy ? "Saving…" : liked ? "Liked" : "Like")}
      </button>
      <button type="button" disabled={loading} onClick={showPeople} aria-label={compact ? `Liked by ${state.count} ${state.count === 1 ? "person" : "people"}. View people.` : undefined} className={compact ? "min-h-10 cursor-pointer rounded-r-xl border border-white/15 bg-white/[0.03] px-3 text-sm text-slate-200 hover:bg-white/10 disabled:cursor-default" : "min-h-10 cursor-pointer text-xs text-[var(--text-muted)] underline decoration-[var(--border)] underline-offset-4 hover:text-[var(--foreground)] disabled:cursor-default"}>{compact ? state.count : `Liked by ${state.count} ${state.count === 1 ? "person" : "people"}`}</button>
    </div>
    ...
    <dialog ref={dialog} aria-labelledby="ranking-likers-title" className="fixed inset-0 m-auto max-h-[80dvh] w-[calc(100%-2rem)] max-w-sm rounded-2xl border border-[var(--border)] bg-[var(--background)] p-5 text-[var(--foreground)] shadow-xl backdrop:bg-black/50 backdrop:backdrop-blur-sm">
      <div className="mb-5 flex items-center justify-between"><h2 id="ranking-likers-title" className="font-semibold">Liked by</h2><button type="button" onClick={() => dialog.current?.close()} aria-label="Close likes" className="h-10 w-10 cursor-pointer rounded-full border border-[var(--border)]">×</button></div>
