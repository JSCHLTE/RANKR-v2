...
export function GameOddsCard({ game, season, week, pass }: { game: GameSummary; season: number; week: number; pass?: RankrPass }) {
  ...
  return <Link href={hasActiveRankrPass(pass) ? `/odds/nfl/${season}/week-${week}/${game.slug}` : "/subscribe"} className="block rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 transition-colors hover:border-[var(--accent)] focus-visible:outline-2 focus-visible:outline-[var(--accent)] sm:p-5">
