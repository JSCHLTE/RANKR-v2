    ...
    <nav aria-label="Main navigation" className="sticky top-0 z-50 w-full border-b border-[var(--border)] px-5 py-4 sm:px-6">
      ...
      <div className="mx-auto flex max-w-200 items-center justify-between">
        ...
        <div className="hidden items-center gap-6 md:flex">
          {navigation.map(link => <Link key={link.href} href={link.href === "/odds" && !loading && !user ? "/signup" : link.href} aria-current={(pathname === link.href || (link.href === "/odds" && pathname.startsWith("/odds/"))) ? "page" : undefined} onClick={() => setUserMenu(false)} className={`text-sm transition-colors ${(pathname === link.href || (link.href === "/odds" && pathname.startsWith("/odds/"))) ? "font-medium text-[var(--accent)]" : "text-[var(--text-muted)] hover:text-[var(--foreground)]"}`}>{link.label}</Link>)}
