  ...
  return (
    <div className="w-full">
      <div className="relative">
        ...
        />
        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-white/20 text-xs select-none">
          {`${value.length}/16`}
        </span>
      </div>
      {status === "checking" && <p className="text-xs mt-1 text-white/40">Checking...</p>}
      {status === "taken" && <p className="text-xs mt-1 text-red-400">Username already taken</p>}
      {status === "available" && <p className="text-xs mt-1 text-green-400">✓ Available</p>}
      {status === "invalid" && <p className="text-xs mt-1 text-red-400">3–16 chars, must start with a letter</p>}
    </div>
  )
};
