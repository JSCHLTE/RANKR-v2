...
export async function syncOdds(season: number, week: number) {
  ...
  const unavailableSportsbooks: Sportsbook[] = [];
  ...
  return { success: true, season, week, gamesFetched: events.length, gamesWritten: games.length,
    propsWritten: games.reduce((sum, game) => sum + game.playerProps.length, 0), sportsbooksFound, unavailableSportsbooks, updatedAt };
}
